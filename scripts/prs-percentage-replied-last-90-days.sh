...
PRS_CREATED_LAST_90_DAYS=$(gh pr list --search "${FILTERED_OUT_LABELS} ${FILTERED_OUT_USERS} created:${DATE_90_DAYS_AGO}..${DATE_TODAY}" --state all --limit 1000 --json number,mergedAt --jq '[.[] | select(.mergedAt != null) | .number]')
