...
do
  if [ "$pr" = "[" ] || [ "$pr" = "]" ]; then
    ...
    PR_PREVIEW_COMMENTS_BY_MAINTAINERS=$(jq --arg DATE "$THREE_DAYS_AFTER_PR_OPENED" '[.[] | select(.author_association == "CONTRIBUTOR" and .author.login != "apollo-cla" and .author.login != "github-actions" and .created_at <= $DATE)] | length' <<< $PR_REVIEW_COMMENTS)
    ...
    if [ "$PR_COMMENTS" = "0" ] && [ "$PR_PREVIEW_COMMENTS_BY_MAINTAINERS" = "0" ]; then
