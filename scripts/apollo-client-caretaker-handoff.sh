...
PD_OUTGOING=$(curl --request GET \
  --url "https://api.pagerduty.com/schedules/${PD_ORG_ID}/users?since=${DATE_2_DAYS_AGO}&until=${DATE_YESTERDAY}" \
  --header 'Accept: application/json' \
  --header "Authorization: Token token=${PD_TOKEN}" \
  --header 'Content-Type: application/json')

PD_INCOMING=$(curl --request GET \
  --url "https://api.pagerduty.com/schedules/${PD_ORG_ID}/users?since=${DATE_TOMORROW}&until=${DATE_2_DAYS_IN_FUTURE}" \
  --header 'Accept: application/json' \
  --header "Authorization: Token token=${PD_TOKEN}" \
  --header 'Content-Type: application/json')
